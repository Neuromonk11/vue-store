<template>
  <form
    id="checkoutForm"
    class="grid gap-4 p-5 border lg:border-0 lg:border-t lg:px-0"
    @submit.prevent='formSubmit'
  >
    <VFieldset legend="Shipping adress">
      <VSelect
        class='col-span-3 sm:col-span-6'
        placeholder='City'
        :textError="textErrorCity"
        :options="optionsCities"
        :selected="formCity"
        @select="$store.commit('updateCity', $event)"
      />
      <VTextFieldB
        class='col-span-3 sm:col-span-6'
        placeholder='Adress'
        :textError="textErrorAddress"
        :value="formAddress"
        @input="$store.commit('updateAddress', $event)"
      />
    </VFieldset>
    <VFieldset legend="Contact">
      <VTextFieldB
        placeholder='Email'
        class='col-span-3 sm:col-span-6'
        :textError="textErrorEmail"
        :value="formEmail"
        @input="$store.commit('updateEmail', $event)"
      />
      <VTextFieldB
        placeholder='Phone number'
        class='col-span-3 sm:col-span-6'
        :textError="textErrorPhone"
        :value="formPhone"
        @input="$store.commit('updatePhone', $event)"
      />
      <VTextFieldB
        placeholder='Surname'
        class='col-span-2 sm:col-span-6'
        :textError="textErrorSurname"
        :value="formSurname"
        @input="$store.commit('updateSurname', $event)"
      />
      <VTextFieldB
        placeholder='Name'
        class='col-span-2 sm:col-span-6'
        :textError="textErrorName"
        :value="formName"
        @input="$store.commit('updateName', $event)"
      />
    </VFieldset>
    <VFieldset legend="Some description">
      <VTextFieldB
        class='col-span-6'
        placeholder='Comment under order'
        :textError="textErrorComment"
        :value="formComment"
        @input="$store.commit('updateComment', $event)"
      />
    </VFieldset>
  </form>
</template>

<script>
import VSelect from '@/components/base/VSelect.vue';
import VTextFieldB from '@/components/base/VTextFieldB.vue';
import VFieldset from '@/components/base/VFieldset.vue';
import { mapGetters } from 'vuex';
import { required, email, minLength, maxLength } from 'vuelidate/lib/validators';

export default {
  name: 'TheCartForm',
  components: {
    VTextFieldB, VSelect, VFieldset,
  },
  validations: {
    formCity: { selected: (v) => v !== null },
    formAddress: { required, maxLength: maxLength(32) },
    formEmail: { required, email, maxLength: maxLength(32) },
    formPhone: { required, minLength: minLength(9), maxLength: maxLength(32) },
    formSurname: { maxLength: maxLength(32) },
    formName: { required, maxLength: maxLength(32) },
    formPatr: { maxLength: maxLength(32) },
    formComment: { maxLength: maxLength(128) },
  },
  computed: {
    ...mapGetters(['formCity', 'formAddress', 'formEmail', 'formPhone', 'formSurname', 'formName', 'formPatr', 'formComment']),
    textErrorCity() {
      if (this.$v.formCity.$dirty) {
        if (!this.$v.formCity.selected) return 'Required field';
      }
      return '';
    },
    textErrorAddress() {
      if (this.$v.formAddress.$dirty) {
        if (!this.$v.formAddress.required) return 'Required field';
        if (!this.$v.formAddress.maxLength) return `Max. lenght ${this.$v.formAddress.$params.maxLength.max} symbols`;
      }
      return '';
    },
    textErrorEmail() {
      if (this.$v.formEmail.$dirty) {
        if (!this.$v.formEmail.required) return 'Required field';
        if (!this.$v.formEmail.email) return 'Wrong email';
        if (!this.$v.formEmail.maxLength) return `Max. lenght ${this.$v.formEmail.$params.maxLength.max} symbols`;
      }
      return '';
    },
    textErrorPhone() {
      if (this.$v.formPhone.$dirty) {
        if (!this.$v.formPhone.required) return 'Required field';
        if (!this.$v.formPhone.minLength) return `Min. lenght ${this.$v.formPhone.$params.minLength.min} symbols`;
        if (!this.$v.formPhone.maxLength) return `Max. lenght ${this.$v.formPhone.$params.maxLength.max} symbols`;
      }
      return '';
    },
    textErrorSurname() {
      if (this.$v.formSurname.$dirty) {
        if (!this.$v.formSurname.maxLength) return `Max. lenght ${this.$v.formSurname.$params.maxLength.max} symbols`;
      }
      return '';
    },
    textErrorName() {
      if (this.$v.formName.$dirty) {
        if (!this.$v.formName.required) return 'Required field';
        if (!this.$v.formName.maxLength) return `Max. lenght ${this.$v.formName.$params.maxLength.max} symbols`;
      }
      return '';
    },
    textErrorPatr() {
      if (this.$v.formPatr.$dirty) {
        if (!this.$v.formPatr.maxLength) return `Max. lenght ${this.$v.formPatr.$params.maxLength.max} symbols`;
      }
      return '';
    },
    textErrorComment() {
      if (this.$v.formComment.$dirty) {
        if (!this.$v.formComment.maxLength) return `Max. lenght ${this.$v.formComment.$params.maxLength.max} symbols`;
      }
      return '';
    },
    optionsCities() {
      return this.$store.getters.cities.map((item) => ({
        value: item.id,
        text: item.name,
      }));
    },
  },
  methods: {
    async formSubmit() {
      if (!this.$v.$invalid) {
        await this.$store.dispatch('addOrder');
        this.$store.dispatch('clearCart');
        this.$store.commit('openWindowSuccess');
      } else {
        this.$v.$touch();
        this.$toasted.show('Some data pasted wrong');
      }
    },
  },
  async mounted() {
    await this.$store.dispatch('fetchCities');
  },
};
</script>
