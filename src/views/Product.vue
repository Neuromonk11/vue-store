  ...
  >
    <div class="grid grid-cols-3 gap-8 mt-2">
      ...
      <div class="col-span-1 lg:col-span-3 lg:mb-4">
        ...
        <div class="font-medium uppercase mb-7 text-5 text-black-70">{{ $store.getters.productPrice }} eur</div>
        <div class="mb-7">
        ...
        <div class="mb-6">
          <div class="mb-2 font-bold uppercase text-theme">Description</div>
          <div class="font-medium">{{ $store.getters.productDescription }}</div>
        </div>
        <VButton @click="addToCart()">Add to Cart</VButton>
      </div>
    </div>
  </VContainer>
  <VNotFound v-else />
  ...
  methods: {
    ...
    async addToCart() {
      if (this.$store.getters.productSelectedSize) {
        ...
        this.$toasted.show('Product was added to cart');
      } else {
        this.$toasted.show('Choose size');
      }
    },
  },
  async mounted() {
